...
<html>
<head>
	<title>Изменение записи в справочнике</title>
...
<%
	...
	EntityValue entityValue = (EntityValue) request.getAttribute("entityValue");
%>
		<h4>Изменение записи в справочнике &laquo;<%=entity.getName()%>&raquo;</h4>
...
%>                  
		<form action="<%=request.getContextPath()%>/entityValueEdit" method="post">
			<input type="hidden" name="entityId" value="<%=entity.getId()%>"/>
			<input type="hidden" name="entityValueId" value="<%=entityValue.getId()%>"/>
			<p>
